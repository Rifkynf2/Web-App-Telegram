...
const MOCK_CATALOG = [
  {
    id: 'm1',
    name: 'Gsuite x Pay',
    description: 'Private Region UK. Hitungan durasi dimulai langsung saat transaksi sukses. PSC = PaysafeCard',
    image_url: '',
    category: 'GENERAL',
    stock_count: 3106,
    variants: [
      {
        ...
        min_qty: 1,
        max_qty: 100,
        description: 'Private Region UK dengan PSC',
        total_sold: 18420,
        last_restock_at: new Date(Date.now() - 2 * 86400000).toISOString(),
        wholesale_tiers: [
          { min_qty: 10, price: 500 },
          { min_qty: 50, price: 400 },
        ],
      },
      { id: 'mv2', name: '3D PSC FR', price: 650, stock: 941, min_qty: 1, max_qty: 100, description: 'Private Region FR dengan PSC', total_sold: 11958, last_restock_at: new Date(Date.now() - 5 * 86400000).toISOString(), wholesale_tiers: [] },
    ],
  },
  {
    id: 'm2',
    name: 'Mail Fresh',
    description: 'Email baru siap pakai, verified dan aman digunakan.',
    image_url: '',
    category: 'EMAIL',
    stock_count: 0,
    variants: [{ id: 'mv3', name: 'Gmail Fresh', price: 1250, stock: 0, min_qty: 1, max_qty: 50, description: null, total_sold: 9341, last_restock_at: new Date(Date.now() - 14 * 86400000).toISOString() }],
  },
  {
    id: 'm3',
    name: 'Alight Motion',
    description: 'Aplikasi edit video profesional dengan semua fitur premium unlocked.',
    image_url: '',
    category: 'APLIKASI',
    stock_count: 114,
    variants: [{ id: 'mv4', name: 'Premium 1 Bulan', price: 2000, stock: 114, min_qty: 1, max_qty: 10, description: 'Akses penuh semua fitur selama 30 hari', total_sold: 4210, last_restock_at: new Date(Date.now() - 1 * 86400000).toISOString() }],
  },
  {
    id: 'm4',
    name: 'Apple Music',
    description: 'Nikmati jutaan lagu tanpa iklan dengan kualitas audio lossless.',
    image_url: '',
    category: 'MUSIK',
    stock_count: 0,
    variants: [{ id: 'mv5', name: 'Individual 3 Bulan', price: 5000, stock: 0, min_qty: 1, max_qty: 5, description: null, total_sold: 7892, last_restock_at: new Date(Date.now() - 20 * 86400000).toISOString() }],
  },
  {
    id: 'm5',
    name: 'Netflix Premium',
    description: 'Streaming film dan serial tanpa batas di semua perangkat.',
    image_url: '',
    category: 'STREAMING',
    stock_count: 57,
    variants: [
  ...
  },
  {
    ...
    description: 'Dengarkan musik favoritmu tanpa iklan, bisa download offline.',
    image_url: '',
    category: 'MUSIK',
    stock_count: 200,
    variants: [{ id: 'mv8', name: 'Individual 1 Bulan', price: 8000, stock: 200, min_qty: 1, max_qty: 10, description: null, total_sold: 12500, last_restock_at: new Date(Date.now() - 0.5 * 86400000).toISOString() }],
  },
];
...
const detailPagePriceOriginal = document.getElementById('detail-page-price-original');
...
const detailPageWholesaleBox = document.getElementById('detail-page-wholesale-box');
const detailPageWholesaleList = document.getElementById('detail-page-wholesale-list');
...
const detailPageTotalOriginal = document.getElementById('detail-page-total-original');
...
const detailPageTotalDeskOriginal = document.getElementById('detail-page-total-desk-original');
...
function selectDetailVariant(variant, chipElement) {
  ...
  // Harga Grosir box
  if (detailPageWholesaleBox && detailPageWholesaleList) {
    const tiers = Array.isArray(variant.wholesale_tiers) ? variant.wholesale_tiers : [];
    const basePrice = parseInt(variant.price, 10) || 0;
    if (tiers.length > 0) {
      detailPageWholesaleList.innerHTML = tiers
        .map((tier) => {
          const discountPct = basePrice > 0 ? Math.round((1 - tier.price / basePrice) * 100) : 0;
          return `
                <div class="flex items-center justify-between tier-display-row rounded-lg border border-transparent px-2 py-1.5 -mx-2 transition-colors" data-min-qty="${tier.min_qty}">
                    <span class="text-gray-300">Beli ${tier.min_qty}+ pcs</span>
                    <span class="flex items-center gap-2">
                        <span class="tier-active-badge hidden text-[10px] font-bold text-emerald-300 bg-emerald-500/20 px-1.5 py-0.5 rounded-full">✓ Aktif</span>
                        <span class="text-xs text-gray-500 line-through">${formatCurrency(basePrice)}</span>
                        <span class="font-black text-emerald-300">${formatCurrency(tier.price)}</span>
                        ${discountPct > 0 ? `<span class="text-[10px] font-bold text-red-400 bg-red-500/15 px-1.5 py-0.5 rounded-full">-${discountPct}%</span>` : ''}
                    </span>
                </div>
                `;
        })
        .join('');
      detailPageWholesaleBox.classList.remove('hidden');
    } else {
      detailPageWholesaleList.innerHTML = '';
      detailPageWholesaleBox.classList.add('hidden');
    }
  }
...
function updateDetailQtyDisplay() {
  ...
  if (detailPagePriceOriginal) {
    detailPagePriceOriginal.textContent = formatCurrency(basePrice);
    detailPagePriceOriginal.classList.toggle('hidden', !isDiscounted);
  }
  if (detailPageWholesaleList) {
    // Only the highest tier whose min_qty <= currentQty is "active", mirroring
    // resolveTierPrice()'s own selection logic, so exactly one row is marked
    // even when qty clears multiple tier thresholds at once.
    const tiers = Array.isArray(activeVariant.wholesale_tiers) ? activeVariant.wholesale_tiers : [];
    let activeMinQty = null;
    for (const tier of tiers) {
      if (currentQty >= tier.min_qty) activeMinQty = tier.min_qty;
    }
  ...
  if (detailPageTotalOriginal) {
    detailPageTotalOriginal.textContent = originalTotal;
    detailPageTotalOriginal.classList.toggle('hidden', !isDiscounted);
  }
  if (btnDetailCheckout) {
    btnDetailCheckout.disabled = !canCheckout;
    btnDetailCheckout.style.opacity = canCheckout ? '1' : '0.5';
  }
  ...
  if (detailPageTotalDeskOriginal) {
    detailPageTotalDeskOriginal.textContent = originalTotal;
    detailPageTotalDeskOriginal.classList.toggle('hidden', !isDiscounted);
  }
  if (btnDetailCheckoutDesk) {
    btnDetailCheckoutDesk.disabled = !canCheckout;
...
async function handleCheckout() {
  ...
  if (btnDetailCheckoutDesk) {
    ...
    btnDetailCheckoutDesk.style.opacity = '0.6';
  }
